...
//		request.getRequestURI(); //  /Model2/test.bo 
		...
		System.out.println("(from BoardFrontController.doProcess) requestURI: " + requestURI);
		
		String ctxPath = request.getContextPath(); // 프로젝트 = 컨텍스트
		System.out.println("(from BoardFrontController.doProcess) contextPath: " + ctxPath);
		
		// ctxPath 길이만큼 자르고, 나머지를 가져와랏~~
		String command = requestURI.substring(ctxPath.length());
		System.out.println("(from BoardFrontController.doProcess) command: " + command);
		// 1. 가상 주소 계산 끝 -----------------------------------------
		...
		if(command.equals("/BoardWrite.bo")){
			...
			System.out.println("(from BoardFrontController.doProcess) /BoardWrite.bo 주소 호출됨");
			System.out.println("(from BoardFrontController.doProcess) DB 정보 필요 없음 -> view 페이지로 이동시킬거임");
...
//			forward.setRedirect(isRedirect); // 어떻게 갈건지
			...
			System.out.println("(from BoardFrontController.doProcess) /BoardWriteAction.bo 호출");
			System.out.println("(from BoardFrontController.doProcess) DB 작업 O + 페이지 이동 O");
															//~~Action == pro 페이지니까!!
...
//				bwAction.execute(request, response);
		...
		if(forward != null){
			...
			if(forward.isRedirect()){
				...
				System.out.println("(from BoardFrontController.doProcess) true -> " + forward.getPath()+"로 이동, sendRedirect() 방식으로");
				response.sendRedirect(forward.getPath());
				
			} else {
				// isRedirect가 False일 때, forward() 방식으로 이동
										// 
				System.out.println("(from BoardFrontController.doProcess) false -> " + forward.getPath()+"로 이동, forward() 방식으로");
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("(from BoardFrontController.doGet) doGet() 실행");
		doProcess(request, response);
	}
	
	@Override
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("(from BoardFrontController.doPost) doPost() 실행");
		doProcess(request, response);
	}
